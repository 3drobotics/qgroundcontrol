...
{
    bool enable = true; //lrw _appSettings->enableTaisync()->rawValue().toBool();
    if(enable) {
#if defined(__ios__) || defined(__android__)
...
{
    bool enable = true; // lrw _appSettings->enableTaisyncVideo()->rawValue().toBool();
    if(enable) {
        ...
        // lrw _savedVideoState  = pVSettings->visible();
        //-- Now set it up the way we need it do be
        // lrw pVSettings->setVisible(false);
        ...
        // lrw qgcApp()->toolbox()->videoManager()->setIsTaisync(true);
...
#if defined(__ios__) || defined(__android__)
       // lrw  qgcApp()->toolbox()->videoManager()->setIsTaisync(false);
        if (_taiVideo) {
            _taiVideo->close();
            _taiVideo->deleteLater();
            _taiVideo = nullptr;
        ...
        if(!_savedVideoSource.isValid()) {
            ...
            // lrw pVSettings->setVisible(_savedVideoState);
